  ...
  */
    ...
    let promises = imgs.map(function (img) {
      let promise = new Promise(function (resolved) {
        ...
        image.onload = function () {
          if (config.preloadPercent && typeof config.preloadPercent !== 'function') {
        ...
        image.onerror = image.onabort = function () {
          if (config.defaultImage) {
            ...
            img.setAttribute('origin-src', img.getAttribute('src'))
            img.setAttribute('src', config.defaultImage)
            Promise.reject(img)
          } else {
            resolved(img)
          }
  ...
  function waterfall(config) {
    ...
    function appendElement (nodeLists, newFlag = false) {
      ...
      if (!config.state.loaded && config.preloadImage) {
        preloadImage(nodeLists, config, eleAppendToMin, nodeLists, newFlag)
      } else {
